-- Fix: Scryfall imports can collapse distinct prints into the same catalog_items row
-- because create_catalog_item_via_api previously slugified only p_item_name.
-- Include identifier in the generated slug when present so separate printings
-- with collector-number edge cases do not overwrite each other.
...
CREATE OR REPLACE FUNCTION public.create_catalog_item_via_api(
  ...
  p_people          jsonb   DEFAULT NULL,
  p_minifigures     jsonb   DEFAULT NULL,
